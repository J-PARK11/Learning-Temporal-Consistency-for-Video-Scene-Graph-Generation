...
class AG(Dataset):
    ...
    def __init__(self, mode, datasize, data_path='/data/AG/', filter_nonperson_box_frame=True, filter_small_box=False):
        ...
        print(f'[Action Genome] {mode}  Object Classes: {len(self.object_classes)}')
        ...
        # Filtering Small Box or Not.
        if filter_small_box:
            with open(root_path + 'annotations/person_bbox.pkl', 'rb') as f:
                person_bbox = pickle.load(f)
            f.close()
            with open('dataloader/AG/object_bbox_and_relationship_filtersmall.pkl', 'rb') as f:
...
if __name__ == '__main__':
    ...
    dataset = AG(mode='test', datasize='mini', filter_nonperson_box_frame=True, filter_small_box=False)
    img_tensor, im_info, gt_boxes, num_boxes, index = dataset[100] #67
