...
class transformer(nn.Module):
    ...
    def forward(self, features, im_idx, memory = [], len_person=None):
        
        # Embedding: RPN Features + BBox Coordinate Embedding + ??? : (Total Token, 968)
        
        # rel_input = Pairwise Feature를 Framewise Feature로 변환하는 단계.
        rel_idx = torch.arange(im_idx.shape[0]) # (Total_Token)

        l = torch.sum(im_idx == torch.mode(im_idx)[0])  # the highest Token Number in the single frame
        b = int(im_idx[-1] + 1)     # Last Im idx
        rel_input = torch.zeros([l, b, features.shape[1]]).to(features.device)  # (l, Frame, 968)
        
        # TODO Padding/Mask maybe don't need for-loop
        masks = torch.zeros([b, l], dtype=torch.bool).to(features.device)       # (Frame, l)
        for i in range(b):
            rel_input[:torch.sum(im_idx == i), i, :] = features[im_idx == i]
            masks[i, torch.sum(im_idx == i):] = 1

        # spatial encoder
        local_output, local_attention_weights = self.local_attention(rel_input, masks)  # (l, Frame, 968), (1, Frame, l, l)
        local_output = (local_output.permute(1, 0, 2)).contiguous().view(-1, features.shape[1])[masks.view(-1) == 0]    # (Total Token, 968)
        """
        local_output: (Pair, 968), local_attention_weights: (1, Frames, 3, 3)
        """
        
        mem_encoder_features = local_output 
            
        global_input = torch.zeros([l * 2, b - 1, features.shape[1]]).to(features.device)   # (l*2, Frame, 968)
        position_embed = torch.zeros([l * 2, b - 1, features.shape[1]]).to(features.device) # (l*2, Frame, 968)
        idx = -torch.ones([l * 2, b - 1]).to(features.device)                               # (l*2, Frame)
        idx_plus = -torch.ones([l * 2, b - 1], dtype=torch.long).to(features.device)        # (l*2, Frame)
        ...
        global_masks = (torch.sum(global_input.view(-1, features.shape[1]),dim=1) == 0).view(l * 2, b - 1).permute(1, 0) # (Frame, l*2)
        
        # temporal decoder
        global_output, global_attention_weights = self.global_attention(global_input, global_masks, position_embed)
                                                                # (l*2, Frame, 968), (Frame, l*2), (l*2, Frame, 968)        
        output = torch.zeros_like(features)
        #=> global_output: (l*2, Frame, 968), global_attention_weights: (3, Frame, l*2, l*2), output: (Pair, 968)
        
        # Parallel to Serial Structure
        if self.mode == 'both':
            # both
            for j in range(b - 1):
                if j == 0:
        ...
        # Dividing for Only Object Node Output
        # dividor = (output.shape[0] - len_person)//2
        # output = output[len_person:len_person+dividor, :]
