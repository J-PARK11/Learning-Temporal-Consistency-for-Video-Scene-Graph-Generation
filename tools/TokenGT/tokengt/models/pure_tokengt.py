...
class TokenGTEncoder(FairseqEncoder):
    def __init__(self, args):
        ...
        if self.load_softmax:
            self.lm_output_learned_bias = nn.Parameter(torch.zeros(26))    # 1
            if not self.share_input_output_embed:
                self.embed_out = nn.Linear(args.encoder_embed_dim, 26, bias=False) # args.num_classes
            else:
                raise NotImplementedError
    ...
    def forward(self, batched_data, perturb=None, masked_tokens=None, **unused):
        ...
        x = inner_states[-1].transpose(0, 1)  # Batch=Frame x Token x Embed
        ...
        if self.share_input_output_embed and hasattr(
            ...
            x = self.embed_out(x)                   # (B, 2+T, 768) => (B, 2+T, 26)
        ...
        out = []
        for row, valid_count in zip(x, object_idx):
            obj_row = row[3:3+int(valid_count)]
            out.append(obj_row)
        out = torch.cat(out, dim=0).to(torch.device('cuda:0'))
