...
class GraphFeatureTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.temp_encoder = nn.Embedding(100, hidden_dim, padding_idx=0)
        self.edge_encoder = nn.Embedding(5, hidden_dim, padding_idx=0)
        ...
        if self.type_id:
            self.order_encoder = nn.Embedding(3, hidden_dim)    # Node인지 Edge (1, 0) => (768)
    ...
    def get_batch(node_feature, edge_index, edge_feature, node_num, edge_num, perturb=None):
        ...
        """
        seq_len = [int(n) + int(e) for n, e in zip(node_num, edge_num)]     # Frame별 총 토큰 수
        b = len(seq_len)                                                    # Frame 수
        d = node_feature.size(-1)                                           # Embedding Dimension
        max_len = max(seq_len)                                              # 토큰의 최대 수
        max_n = max(node_num)                                               # 노드의 최대 수
    ...
    @staticmethod
    def get_index_embed(node_id, node_mask, padded_index):  # 토큰 인덱스가 부여되는 부분. Node Identifier
        # 노드와 에지의 Node Identifier를 Dot Product했을 때, 연결 되어 있으면 Product 결과가 1이다.
        """
    ...
    def get_type_embed(self, padded_index): # 노드인지 에지인지 구분하는 함수: Type Identifier
        """
        :param padded_index: LongTensor([B, T, 2])
        :return: Tensor([B, T, D])
        """
    ...
    def add_special_tokens(self, padded_feature, padding_mask): # 추가로 그래프 토큰 혹은 Null 토큰 혹은 타임 토큰이 추가되는 부분 # , target
        """
        ...
        num_special_tokens = 2 #target.shape[0] #2
    ...
    def forward(self, batched_data, perturb=None):
        (
            ...
            batched_data['temporal_pe'],
            batched_data["edge_index"],
            batched_data["edge_data"],
            batched_data["edge_num"]
        )

        frame_idx = frame_idx-min(frame_idx)    # VidVRD
        node_feature = self.atom_encoder(node_data)            # [sum(n_node), D]
        self.temporal_pe = self.temp_encoder(frame_idx).sum(-2)
        
        node_feature = node_feature + self.temporal_pe        
        ...
        org_padded_index = padded_index.clone()
        ...
        if self.type_id:
            padded_feature = padded_feature + self.get_type_embed(org_padded_index)

        padded_feature, padding_mask = self.add_special_tokens(padded_feature, padding_mask)  # [B, 2+T, D], [B, 2+T] #, batched_data['target']

        padded_feature = padded_feature.masked_fill(padding_mask[..., None], float('0'))
        return padded_feature, padding_mask, org_padded_index, node_mask  # [B, 2+T, D], [B, 2+T], [B, T, 2], [B, T]
