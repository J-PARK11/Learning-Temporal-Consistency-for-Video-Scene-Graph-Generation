...
from fairseq.modules.layer_norm import LayerNorm, LayerDropModuleList
...
class TokenGTGraphEncoder(nn.Module):
        ...
        self.layers.extend(
            [
                self.build_tokengt_graph_encoder_layer(
                    ...
                    drop_path=(0. * (layer_idx + 1) / num_encoder_layers) if stochastic_depth else 0,
                    performer=performer,
    ...
    def forward(
        ...
        if token_embeddings is not None:
            ...
            x, padding_mask, padded_index, node_mask = self.graph_feature(batched_data, perturb)
        ...
        for i in range(len(self.layers)):   # 12
            layer = self.layers[i]
        ...
        if self.traceable:
            ...
            return inner_states, graph_rep, attn_dict, node_mask 
