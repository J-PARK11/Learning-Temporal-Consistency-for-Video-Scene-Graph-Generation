...
def visualize(args, checkpoint_path=None, logger=None, save_path=''):
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(progress):
            ...
            print("Node Config")
            print(f'net_input: {sample["net_input"].keys()}')
            print(f'batched_data: {sample["net_input"]["batched_data"].keys()}')
            
            print(f'node_idx: {sample["net_input"]["batched_data"]["idx"]}')
            print(f'node_data: {sample["net_input"]["batched_data"]["node_data"]}')
            print(f'in_degree: {sample["net_input"]["batched_data"]["in_degree"]}')
            print(f'out_degree: {sample["net_input"]["batched_data"]["out_degree"]}')
            print(f'y: {sample["net_input"]["batched_data"]["y"]}')
            print(f'node_num: {node_num}')
            print(f'lap_eigvec: {sample["net_input"]["batched_data"]["lap_eigvec"]}')
            print(f'lap_eigval: {sample["net_input"]["batched_data"]["lap_eigval"]}')
            print('\nEdge Config')
            print(f'edge_index: {sample["net_input"]["batched_data"]["edge_index"]}')
            print(f'edge_data: {sample["net_input"]["batched_data"]["edge_data"]}')
